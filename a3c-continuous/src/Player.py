...
class Player(object):
	...
	def __init__(self, cfg, ckpt_path):
		...
		self.model = ActorCriticModel(self.cfg)
		self.model.eval()
	...
	def play(self):
		...
		while not self.env.done:
			...
			_, policy_mu, _, model_state = self.model(Variable(state.unsqueeze(0)), model_state)
	...
	def play_n(self, num_games):
		...
		for game in range(num_games):
			...
			while not self.env.done:
				...
				_, policy_mu, _, model_state = self.local_net(Variable(state.unsqueeze(0)), model_state)
...
if __name__ == '__main__':
	...
	player = Player(cfg, args.ckpt_path)
